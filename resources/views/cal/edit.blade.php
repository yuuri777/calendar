...
<div class="d-flex">
    ...
    <div class="container" >
        <div class="row justify-content-center" >
            <div class="col-md-8">
                <div class="card" >
                    <div class="card-header">経済指標編集</div>

                    <div class="card-body">
                    @foreach ($cal as $item)
                        <form method="POST" action="{{ route('cal.update', $item['id']) }}">
                            ...
                            <div class="form-group row">
                                ...
                                <select name="importance" id="task_status" class="form-select @error('task_status')is-invalid @enderror">
                                    ...
                                    @foreach($calStatusStrings as $key => $calstatus)
                    ...
                    @endforeach
                    @foreach ($cal as $item)
                        <form action="{{route('cal.delete', $item['id']) }}" method="post">
                        @csrf
                        @method('DELETE')
                            <button class="btn create ml-2" >削除</button> 
                        </form>
                    </div>
                </div>
                    @endforeach
